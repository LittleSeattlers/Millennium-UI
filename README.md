...
The GitHub Pages site cannot install native software silently. Complete this setup once in **Windows PowerShell** as your normal Windows user.

1. Install Git, GitHub CLI, and the current Node.js LTS release:
...
2. Close every PowerShell window and open a new one so Windows refreshes `PATH`. Install the pinned pnpm release and verify the prerequisites. Millennium requires Node.js 22.13 or newer and pnpm 11:
...
3. Clone the repository and install its pinned dependencies, including the Codex CLI used by the connector:

```powershell
git clone https://github.com/LittleSeattlers/Millennium-UI.git
Set-Location .\Millennium-UI
pnpm install
```

4. Sign in to GitHub, then sign the bundled Codex CLI into the ChatGPT subscription that should fund Millennium runs. For `gh auth login`, choose GitHub.com, HTTPS, and browser sign-in. For Codex, choose **Sign in with ChatGPT** rather than an API key:
...
5. Install and start the per-user background connector:

```powershell
pnpm connector:install
```

The installer verifies Git, GitHub CLI authentication, and the packaged Codex executable, starts the connector in the background, and registers it to start when you sign in to Windows. No terminal needs to remain open. After setup:
