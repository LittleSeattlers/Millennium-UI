...
export async function startRunner({
  ...
  const server = http.createServer((req, res) => {
    routeRequest({ req, res, manager, allowedOrigins, runnerToken, publicControl, nativeControlToken })
      .catch((error) => sendError(res, error, null));
  });
  ...
  try {
    ...
    if (req.method === 'POST' && pathname === '/api/v1/quota/snapshots') {
      ...
      const quota = await manager.estimate({
        ...
        riskMode: body.riskMode ?? QUOTA_RISK_MODE,
        keepPercent: body.keepPercent ?? QUOTA_RESERVE_PERCENT,
      });
...
function assertPublicSession(req, origin, publicControl) {
  ...
  for (const [expected, session] of publicControl.sessions) {
    ...
    refreshPublicSession(session, now);
    return session;
  }
  throw new HttpError(401, 'The hosted control session is missing, expired, or invalid.', 'unauthorized');
}

export function refreshPublicSession(session, now = Date.now()) {
  session.lastSeenAtMs = now;
  session.expiresAtMs = now + PUBLIC_SESSION_TTL_MS;
  return session;
}
...
async function routePublicRequest({ req, res, manager, url, pathname, headers, session, publicControl }) {
  ...
  if (req.method === 'POST' && pathname === '/api/v1/public/quota/estimate') {
    ...
    const result = await manager.estimate({
      provider: 'codex',
      riskMode: QUOTA_RISK_MODE,
      keepPercent: QUOTA_RESERVE_PERCENT,
...
function publicAttempt(attempt) {
  ...
  return {
    ...
    researchValue: attempt.researchValue ? {
      status: String(attempt.researchValue.status ?? 'unknown'),
      outcome: attempt.researchValue.outcome == null ? null : String(attempt.researchValue.outcome),
      reason: attempt.researchValue.reason == null ? null : sanitizePublicText(String(attempt.researchValue.reason)),
    } : null,
  };
}
...
function publicEvent(event) {
  ...
  const messages = {
    ...
    FINALIZE: 'Codex entered the reserved synthesis phase.',
    VALUE: 'The research-value gate evaluated the result.',
    PUBLISH: 'An evidence-bearing contribution was submitted.',
