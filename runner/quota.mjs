...
export function estimateQuota({
  ...
  weeklyKeepPercent = WEEKLY_QUOTA_RESERVE_PERCENT,
  model = null,
  now = Date.now(),
}) {
  ...
  const weeklyReservePercent = clamp(Number(weeklyKeepPercent), 0, 95);
  const estimates = [];
  ...
  for (const window of snapshot.windows) {
    ...
    const windowReservePercent = isWeeklyQuotaWindow(window)
      ? weeklyReservePercent
      : reservePercent;
    const safePercent = Math.max(0, window.remainingPercent - windowReservePercent - roundingMargin);
    ...
    if (safePercent <= 0) {
      return blocked(`${window.label} has no allowance above the ${windowReservePercent}% reserve.`);
    }
    ...
    estimates.push({
      ...
      reservePercent: windowReservePercent,
      safePercent,
...
function isWeeklyQuotaWindow(window) {
  return Math.abs(Number(window?.windowMinutes) - 10_080) <= 60;
}
