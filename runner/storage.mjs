...
import { deriveAttemptBurnObservations } from './quota.mjs';
import {
  assertIdentifier,
  requireString,
  resolveWithin,
...
export async function loadBurnObservations(provider = null) {
  const [snapshots, attempts] = await Promise.all([
    loadQuotaSnapshots(provider),
    listAttempts({ limit: 2_000 }),
  ]);
  return deriveAttemptBurnObservations({ snapshots, attempts })
    .filter((observation) => !provider || observation.provider === provider);
}
...
export async function listResearchRecords({ problem = null, limit = 2_000 } = {}) {
  ...
  return mergeResearchRecords({ local, shared, contributions })
    .sort((left, right) => Date.parse(right.startedAt) - Date.parse(left.startedAt))
    .slice(0, Math.max(1, Math.min(MAX_SHARED_ATTEMPTS, Number(limit) || 2_000)));
}

export function mergeResearchRecords({ local = [], shared = [], contributions = [] } = {}) {
  ...
  for (const attempt of local) {
    const repositoryRecord = records.get(attempt.attemptId);
    const recoveredProposals = sameAcceptedLocalContribution(attempt, repositoryRecord)
      ? repositoryRecord.proposedTasks ?? []
      : [];
    records.set(attempt.attemptId, {
      ...attempt,
      proposedTasks: attempt.proposedTasks?.length ? attempt.proposedTasks : recoveredProposals,
      recordSource: 'local',
    });
  }
...
function sameAcceptedLocalContribution(local, repositoryRecord) {
  return local?.researchValue?.status === 'accepted'
    && local?.publication?.status === 'submitted'
    && repositoryRecord?.attemptId === local.attemptId
    && repositoryRecord?.problem === local.problem
    && repositoryRecord?.direction === local.direction
    && repositoryRecord?.researchTask?.taskId === local.researchTask?.taskId;
}
...
function publicView(state) {
  ...
  return {
    ...
    proposedTasks: state.proposedTasks ? safeSerializable(state.proposedTasks) : [],
  };
}
