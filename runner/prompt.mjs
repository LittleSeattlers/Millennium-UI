...
- In your first work cycle, create or update WORKLOG.md with the target, assumptions, plan, and first checkpoint, then create truthful initial versions of RESULT.md and CONTRIBUTION.proposed.json before beginning deeper research. The initial result may say that no conclusion exists yet, but it must state the objective, current evidence, limitations, and the next useful action.
...
- CONTRIBUTION.proposed.json is the only research content eligible for automatic publication. It must always remain valid JSON containing exactly these fields: summary, claims, limitations, failed_approaches, next_actions, proposed_tasks, and citations. Keep unsupported or not-yet-established fields as empty arrays; never use placeholders that assert evidence you do not have.
...
- Each proposed_tasks item must contain exactly title, objective, rationale, success_criteria, useful_failure_criteria, verification_method, suggested_minutes (5-120), and relationship ("child", "alternative", or "verification"). Include zero to three.
...
2. Replace CONTRIBUTION.proposed.json with one valid JSON object containing exactly: summary, claims, limitations, failed_approaches, next_actions, proposed_tasks, and citations.
3. Keep every unsupported list empty. Do not invent claims, evidence, verification, citations, or completion. Preserve uncertainty and scope limits explicitly.
4. Do not inspect anything outside the current working directory and do not begin another line of attack.
...
Start with \`WORKLOG.md\`. Before stopping, leave \`RESULT.md\`, \`CONTRIBUTION.proposed.json\`, plus any local derivations, citations, code, certificates, and explicit limitations. The structured contribution is reconstructed and checked against a strict canonical contract before a data-only pull request; every other file remains private on this computer. Do not store hidden chain-of-thought or raw provider/account data.
`;
