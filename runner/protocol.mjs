...
async function spawnConnector(controlToken) {
  const logFile = path.join(path.dirname(connectorStatePath()), 'connector.log');
  await mkdir(path.dirname(logFile), { recursive: true });
  await rotateConnectorLog(logFile);
  const logHandle = await open(logFile, 'a', 0o600);
  await logHandle.appendFile(`[${new Date().toISOString()}] Launching Millennium Connector.\n`, 'utf8');
...
async function rotateConnectorLog(logFile) {
  try {
    const metadata = await stat(logFile);
    if (metadata.size < 4 * 1024 * 1024) return;
