...
import { PUBLIC_SESSION_TTL_MS } from '../constants.mjs';
import { createPublicSession, refreshPublicSession } from '../server.mjs';
...
test('an authenticated public session receives a sliding expiry', () => {
  const control = publicControl();
  const session = createPublicSession(control, 'https://littleseattlers.github.io', 1_000);

  refreshPublicSession(session, 12_000);

  assert.equal(session.lastSeenAtMs, 12_000);
  assert.equal(session.expiresAtMs, 12_000 + PUBLIC_SESSION_TTL_MS);
});
