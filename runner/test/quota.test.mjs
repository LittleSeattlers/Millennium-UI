...
test('defaults to five-percent five-hour and one-percent weekly reserves', () => {
  const estimate = estimateQuota({
    snapshot: snapshot('2026-09-08T17:09:00.000Z', 73),
    observations: [],
    model: 'gpt-5.6-sol',
    now: NOW,
  });
  ...
  assert.equal(estimate.reservePercent, 5);
  assert.equal(estimate.weeklyReservePercent, 1);
  assert.equal(estimate.windows.find((window) => window.label === '5-hour window')?.reservePercent, 5);
  assert.equal(estimate.windows.find((window) => window.label === 'Weekly window')?.reservePercent, 1);
});
