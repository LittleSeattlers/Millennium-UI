...
test('a completed preparatory slice is not regenerated without a concrete successor', async (t) => {
  const value = await fixture();
  t.after(() => fs.rm(value.root, { recursive: true, force: true }));
  const parent = { ...value.task, suggestedMinutes: 30 };
  await fs.writeFile(value.catalogFile, JSON.stringify({ schemaVersion: 1, tasks: [parent] }));
  ...
  assert.equal(completed.tasks.some((task) => task.id.startsWith(`${slice.id}.`)), false);
  assert.equal(completed.recommendedTaskId, null);
});

test('an accepted preparatory slice promotes its concrete successor instead of repeating generic decomposition', async (t) => {
  const value = await fixture();
  t.after(() => fs.rm(value.root, { recursive: true, force: true }));
  const parent = { ...value.task, suggestedMinutes: 30 };
  await fs.writeFile(value.catalogFile, JSON.stringify({ schemaVersion: 1, tasks: [parent] }));
  ...
  const first = initial.tasks.find((task) => task.id.startsWith('auto.slice.v1.'));
  const sourceAttemptId = 'navier-stokes-slice-attempt-0001';
  const childId = 'community.0123456789abcdef';
  const attempts = [{
    id: sourceAttemptId,
    problemId: 'ns',
    direction: 'blowup',
    status: 'completed',
    researchValue: { status: 'accepted', outcome: 'frontier-refinement' },
    researchTask: { taskId: first.id, branchId: first.branchId },
    proposedTasks: [
      {
        id: childId,
        title: 'Prove one endpoint-rate sufficiency lemma',
        objective: 'Prove one quantified endpoint-rate implication and audit every continuity and norm assumption needed by the contradiction.',
        rationale: 'The completed decomposition identified endpoint-approaching information as the smallest unresolved analytic dependency.',
        success_criteria: 'A quantified implication with every domain, constant, limit, and continuation assumption explicit.',
        useful_failure_criteria: 'A counterexample under the stated assumptions or the exact additional hypothesis required for the implication.',
        verification_method: 'Independently rederive the endpoint contradiction and test all zero and nonuniform boundary cases.',
        suggested_minutes: 30,
        relationship: 'child',
        sourceAttemptIds: [sourceAttemptId],
      },
      {
        id: 'community.abcdef0123456789',
        title: 'Build a larger terminal interval certificate',
        objective: 'Specify a finite interval certificate and every analytic remainder condition required to cover the complete terminal interval.',
        rationale: 'This is a distinct but substantially larger dependency exposed by the same accepted decomposition.',
        success_criteria: 'A complete finite certificate schema with every interval and asymptotic remainder explicit.',
        useful_failure_criteria: 'The exact terminal coverage obstruction and the smallest additional asymptotic datum it requires.',
        verification_method: 'Independently check every interval inclusion and the limiting terminal coverage argument.',
  ...
  const child = frontier.tasks.find((task) => task.id === childId);
  const nextSlice = frontier.tasks.find((task) => task.parentTaskId === childId
    && task.id.startsWith('auto.slice.v1.'));
  assert.equal(child?.status, 'available');
  assert.equal(frontier.recommendedTaskId, nextSlice?.id);
  assert.equal(frontier.tasks.some((task) => task.id.startsWith(`${first.id}.`)), false);
  ...
  assert.equal(prepared.parentTaskId, childId);
  assert.deepEqual(prepared.sourceAttemptIds, [sourceAttemptId]);
});
