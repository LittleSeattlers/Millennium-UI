...
import { stableProposedTaskId } from '../scripts/contribution-contract.mjs';
...
export class AttemptManager {
  ...
  async providerState() {
    ...
    for (const provider of PROVIDERS) {
      ...
      result.push({
        ...status,
        snapshot: latest,
        estimate: await this.#estimateFrom(
          provider,
          latest,
          snapshots,
          'balanced',
  ...
  async estimate({ provider, riskMode = QUOTA_RISK_MODE, keepPercent = QUOTA_RESERVE_PERCENT, model: requestedModel = null } = {}) {
    ...
    const model = requestedModel == null
      ? null
      : requireString(requestedModel, 'model', { min: 1, max: 160 });
    ...
    return {
      provider,
      snapshot,
      estimate: await this.#estimateFrom(provider, snapshot, snapshots, riskMode, keepPercent, model),
    };
  }
  ...
  async retryPublication(id) {
    ...
    if (!prepared.publishable || !prepared.record) {
      await cancelClaim(state.publicationHandle).catch(() => {});
      ...
      await appendPublicEvent(id, {
        kind: 'VALUE',
        level: 'warning',
        message: 'No research contribution was added to the shared ledger.',
        detail: 'The local files remain recoverable and the task is available for another attempt.',
      });
      return this.get(id);
    }
    try {
      ...
      await updateAttemptState(id, {
        ...
        researchValue: {
          status: 'accepted',
          outcome: prepared.record.value_assessment.outcome,
        },
      });
      await appendPublicEvent(id, {
        kind: 'PUBLISH',
  ...
  async start(body) {
    ...
    try {
      ...
      config.priorContext = await loadPriorContext(config.problem, {
        ...
        branchId: researchTask.branchId,
      });
      ...
      try {
        ...
        active.done.catch((error) => {
          console.error(`Attempt completion escaped its recovery boundary: ${safeError(error)}`);
        });
        active.settleLaunch();
  ...
  #validateStart(body) {
    ...
    const riskMode = requireChoice(body?.riskMode ?? QUOTA_RISK_MODE, Object.keys(RISK_MODES), 'riskMode');
    const keepPercent = requireNumber(body?.keepPercent ?? QUOTA_RESERVE_PERCENT, 'keepPercent', { min: 0, max: 95 });
  ...
  async #estimateFrom(provider, snapshot, snapshots, riskMode, keepPercent, model = null) {
    const attempts = await listAttempts({ limit: 2_000 });
    const observations = deriveAttemptBurnObservations({ snapshots, attempts });
    return estimateQuota({ snapshot, observations, riskMode, keepPercent, model });
  }
  ...
  async #applyQuotaGuard(active, snapshot) {
    ...
    if (this.active !== active || active.stopping || active.finalizing) return;
    const estimate = await this.#estimateFrom(
      active.attempt.provider,
      snapshot,
      history,
      active.attempt.riskMode,
      active.attempt.keepPercent,
      active.attempt.model,
    );
    if (estimate.status !== 'ready') {
      if (!active.handle) active.pendingStopReason = estimate.reason;
      else this.#runBackground(active, 'quota safety stop', () => this.stop(active.attempt.id, estimate.reason));
      return;
    }
  ...
  #armGuards(active, estimate) {
    ...
    const checkpoint = setInterval(() => {
      this.#runBackground(active, 'scheduled checkpoint', async () => {
        try {
          const result = await checkpointAttempt(active.attempt.id, 'scheduled');
          await this.#enqueuePublic(active, {
            kind: 'CHECKPOINT',
          ...
          await this.stop(active.attempt.id, `checkpoint failed: ${safeError(error)}`);
        }
      });
    }, checkpointMs);
    checkpoint.unref?.();
    ...
    const quotaGuard = setInterval(() => {
      ...
      this.#runBackground(active, 'quota guard', async () => {
        const history = await loadQuotaSnapshots(active.attempt.provider);
        const latest = history.at(-1) ?? null;
        const current = await this.#estimateFrom(
          active.attempt.provider,
          latest,
          history,
          active.attempt.riskMode,
      ...
      }, { stopOnError: true, onSettled: () => { active.guardPending = false; } });
    }, 30_000);
    quotaGuard.unref?.();
    ...
    if (active.attempt.provider === 'codex' && active.handle?.refreshQuota) {
      const refresh = setInterval(() => {
        this.#runBackground(active, 'Codex quota refresh', () => active.handle.refreshQuota(), { stopOnError: true });
      }, 2 * 60_000);
      refresh.unref?.();
  ...
  async #finishWhenDone(active) {
    ...
    try {
      ...
      result = { status: 'failed', error: safeError(error), usage: null };
    }
    try {
      return await this.#finish(active, result);
    } catch (error) {
      const detail = safeError(error);
      console.error(`Attempt finalization failed for ${active.attempt.id}: ${detail}`);
      try {
        await finalizeAttempt(active.attempt.id, {
          status: 'failed',
          disposition: 'runner-finalization-failed',
          error: detail,
          finishedAt: new Date().toISOString(),
        ...
        console.error(`Durable finalization recovery failed for ${active.attempt.id}: ${safeError(recoveryError)}`);
      }
  ...
  async #finish(active, result) {
    ...
    active.finalizing = (async () => {
      try {
        ...
        const rawDisposition = classifyTerminalDisposition({
          status: result.status,
          stopReason: active.pendingStopReason,
        });
        let finalStatus = result.status;
        let finalDisposition = rawDisposition;
        let preparedAtDeadline = null;
        if (result.status === 'interrupted' && rawDisposition === 'time-limit') {
          const candidate = await prepareContribution({
            ...(await this.get(active.attempt.id)),
            status: 'completed',
            finishedAt,
          });
          if (shouldCompleteFromDurableDeadline({
            status: result.status,
            disposition: rawDisposition,
            prepared: candidate,
          })) {
            finalStatus = 'completed';
            finalDisposition = 'completed-from-durable-deadline';
            preparedAtDeadline = candidate;
          }
        }
        await finalizeAttempt(active.attempt.id, {
          status: finalStatus,
          disposition: finalDisposition,
          error: finalStatus === 'completed' ? null : result.error ? safeError(result.error) : null,
        ...
        try {
          ...
          const prepared = preparedAtDeadline ?? await prepareContribution(finalized);
          if (!prepared.publishable || !prepared.record) {
            await cancelClaim(active.publicationClaim).catch(() => {});
            ...
            publicationDetail = 'No research-value contribution was submitted; the local files remain recoverable and the task returned to the frontier.';
            await this.#enqueuePublic(active, {
              kind: 'VALUE',
              level: 'warning',
              message: 'Result did not meet the research-value contract.',
              detail: 'It was withheld from the shared ledger, and this task remains available for a better attempt.',
            });
          } else {
            ...
            await updateAttemptState(active.attempt.id, {
              ...
              proposedTasks: prepared.record.proposed_tasks.map((proposal, index) => ({
                id: stableProposedTaskId(prepared.record.attempt_id, proposal, index),
                ...proposal,
                sourceAttemptIds: [prepared.record.attempt_id],
              })),
            });
            publicationDetail = `Contribution PR #${publication.prNumber} passed the local value gate and is ready for automatic data-only validation.`;
            await this.#enqueuePublic(active, {
              kind: 'PUBLISH',
              message: 'Evidence-bearing research contribution submitted.',
        ...
        await this.#enqueuePublic(active, {
          ...
          message: `Attempt ${finalStatus}.`,
          detail: finalStatus === 'completed'
            ? publicationDetail
            : result.error ? safeError(result.error) : publicationDetail,
        });
  ...
  #resetHardStop(active) {
    ...
    active.hardStopTimer = setTimeout(
      () => this.#runBackground(active, 'hard deadline stop', () => this.stop(active.attempt.id, 'safe time budget exhausted')),
  ...
  #resetFinalization(active) {
    ...
    if (delayMs <= 0) {
      ...
      this.#runBackground(active, 'finalization request', () => this.#requestFinalization(active), { stopOnError: true });
      return;
    }
  ...
  #runBackground(active, label, operation, { stopOnError = false, onSettled = null } = {}) {
    const task = Promise.resolve().then(operation);
    task.catch((error) => {
      const detail = safeError(error);
      console.error(`${label} failed for ${active.attempt.id}: ${detail}`);
      if (stopOnError && this.active === active && !active.stopping && !active.finalizing) {
        this.stop(active.attempt.id, `${label} failed: ${detail}`).catch((stopError) => {
          console.error(`Recovery stop failed for ${active.attempt.id}: ${safeError(stopError)}`);
        });
      }
    }).finally(() => {
      try { onSettled?.(); } catch (error) {
        console.error(`${label} cleanup failed for ${active.attempt.id}: ${safeError(error)}`);
      }
    });
    return task;
  }
...
export function shouldCompleteFromDurableDeadline({ status, disposition, prepared } = {}) {
  return status === 'interrupted'
    && disposition === 'time-limit'
    && prepared?.publishable === true
    && prepared?.usedProposal === true
