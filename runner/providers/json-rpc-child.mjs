      ...
      this.#handleLine(line).catch((error) => {
        try {
          this.onNotification?.({
            ...
            params: { message: `Provider event handling failed: ${error instanceof Error ? error.message : 'unknown error'}` },
          });
        } catch { /* the provider is closed below */ }
        void this.close(250).catch(() => {});
      });
    });
