...
export async function readCodexRateLimits(rpc, {
  attempts = 2,
  timeoutMs = 15_000,
  retryDelayMs = 250,
  ...
  for (let attempt = 1; attempt <= attempts; attempt += 1) {
    try {
      return await rpc.request('account/rateLimits/read', {}, timeoutMs);
    } catch (error) {
      lastError = error;
      const message = error instanceof Error ? error.message : String(error);
      if (attempt >= attempts || !/\btimed out\.?$/i.test(message)) throw error;
...
export async function probeCodex({ onSnapshot } = {}) {
  ...
  try {
    ...
    const limits = await readCodexRateLimits(rpc);
  ...
  const rpc = createRpc({
    ...
    onNotification: (message) => handleNotification(message).catch((error) => {
        ...
        Promise.resolve(onEvent({
          kind: 'GUARD',
          level: 'error',
          message: 'A Codex event could not be persisted safely; the attempt is stopping.',
          detail,
        })).catch(() => {});
