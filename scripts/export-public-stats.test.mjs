...
async function addContribution(root, { problem, status, id, problemId = 'ns', direction = 'regular', year = '2026' }) {
  ...
  const record = {
    ...
    value_assessment: {
      ...
      novelty: 'This aggregate fixture is the first stored baseline for its bounded test task.',
      evidence: 'A deterministic fixture observation supplies evidence for the bounded test claim.',
      falsifier: 'A repeated fixture check producing a different value would invalidate the claim.',
    },
    claims: [{
      statement: 'The deterministic fixture completed the declared bounded aggregate test.',
      confidence: 'computational',
      evidence_summary: 'The test fixture records the deterministic completion state for this bounded input.',
      verification_method: 'Re-run the same fixture and compare the complete aggregate state.',
    }],
    limitations: ['This fixture is unreviewed and exists only to test aggregate publication.'],
    failed_approaches: [],
    next_actions: ['Re-run the bounded fixture when the aggregate exporter contract changes.'],
    proposed_tasks: [],
    citations: [],
    prior_attempt_ids: [],
