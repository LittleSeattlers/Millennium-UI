...
test('uses high reasoning for verification even with ample headroom', () => {
  const recommendation = recommendCodexConfiguration({
...
test('keeps verification at high reasoning across the ample-time boundary', () => {
  ...
  assert.equal(atBoundary.effort, 'high');
});
...
test('uses high Sol for an ample verification despite a Terra account default', () => {
  const recommendation = recommendCodexConfiguration({
    models: [{ ...terra, isDefault: true }, { ...sol, isDefault: false }],
    defaultModel: 'gpt-5.6-terra',
    researchMode: 'verify',
...
test('never escalates an automatic recommendation above high reasoning', () => {
  for (const researchMode of ['recommended', 'frontier', 'explore', 'verify']) {
    for (const taskKind of ['proof', 'counterexample-search', 'computation', 'formalization', 'review', 'synthesis', 'exploration']) {
      const recommendation = recommendCodexConfiguration({
        models: [sol, terra, luna],
        researchMode,
        taskKind,
      ...
      assert.notEqual(recommendation.effort, 'xhigh');
    }
  }
});
