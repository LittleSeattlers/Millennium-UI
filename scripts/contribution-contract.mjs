...
export const CONTRIBUTION_SCHEMA_VERSION = 2;
...
const VALUE_OUTCOME_SET = new Set(['scoped-advance', 'bounded-negative', 'reproducibility-result', 'frontier-refinement']);
...
export function buildContributionRecord({ attempt, proposal, sanitizedText = identity }) {
  ...
  const record = {
    ...
    value_assessment: normalizeValueAssessment(
      input.value_assessment ?? input.valueAssessment,
      sanitizedText,
    ),
    ...
    prior_attempt_ids: uniqueIdentifiers(
      attempt.consultedAttemptIds ?? attempt.parentAttemptIds ?? [],
      10,
      'prior_attempt_ids',
    ),
...
export function validateContributionRecord(value) {
  exactObject(value, [
    ...
    'started_at', 'finished_at', 'research_task', 'summary', 'value_assessment', 'claims', 'limitations',
  ...
  canonical(value.value_assessment, normalizeValueAssessment(value.value_assessment), 'value_assessment');
...
function normalizeValueAssessment(value, sanitize = identity) {
  exactObject(value, ['outcome', 'novelty', 'evidence', 'falsifier'], 'value_assessment');
  return {
    outcome: choice(value.outcome, VALUE_OUTCOME_SET, 'value_assessment.outcome'),
    novelty: text(value.novelty, 'value_assessment.novelty', 30, 1_200, sanitize),
    evidence: text(value.evidence, 'value_assessment.evidence', 40, 1_500, sanitize),
    falsifier: text(value.falsifier, 'value_assessment.falsifier', 30, 1_200, sanitize),
  };
}

function assertResearchValue(record) {
  if (record.claims.length === 0) {
    fail('a research contribution must contain at least one bounded, evidence-bearing claim');
  }
  if (record.limitations.length === 0) {
    fail('a research contribution must state at least one explicit limitation');
  }
  if (record.next_actions.length === 0 && record.proposed_tasks.length === 0) {
    fail('a research contribution must leave at least one concrete next action or successor task');
  }
  if (record.value_assessment.outcome === 'frontier-refinement' && record.proposed_tasks.length === 0) {
    fail('a frontier-refinement contribution must propose at least one bounded successor task');
  }
