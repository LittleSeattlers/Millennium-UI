...
export function LocalResearchConsole({ problem, route }: { problem: Problem; route: Route }) {
  ...
  const usefulAllowanceMessage = estimate?.status === 'ready'
    ? `${safeMinutes} safe minute${safeMinutes === 1 ? '' : 's'} ${safeMinutes === 1 ? 'is' : 'are'} available. The shortest useful research run requires ${minimumUsefulRunMinutes} minutes (${usefulMinuteShortfall} more minute${usefulMinuteShortfall === 1 ? '' : 's'}). No Codex research run will start.${estimate.calibration ? ' Use Codex normally, then click Refresh after its quota meter changes to build a burn-rate observation.' : ''}`
              ...
              <div className="composer-controls">
                ...
                <div className="frontier-heading">
                  <div>
                    ...
                    <b>Automatic chooses the best fitting task. Select another research path here when needed.</b>
                  </div>
                  {frontier && (
