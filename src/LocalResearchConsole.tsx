...
const quotaRiskMode = 'harvest' as const;
...
export function LocalResearchConsole({ problem, route }: { problem: Problem; route: Route }) {
  ...
  const safeMinutes = Math.max(0, Math.floor(estimate?.allowedMinutes ?? 0));
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    setEstimate(null);
    retryTransientLocalRequest(() => client.estimate(quotaRiskMode, effectiveModel?.model))
      .then((value) => {
        if (!cancelled) {
          setEstimate(value);
    ...
    return () => { cancelled = true; };
  }, [client, effectiveModel?.model, provider?.ready]);
  ...
  async function refreshCodex() {
    ...
    try {
      ...
      if (refreshed?.ready && refreshedState.health.coordination?.ready) {
        const refreshedEstimate = await retryTransientLocalRequest(() => client.estimate(quotaRiskMode, effectiveModel?.model));
                  ...
                  {!advancedOpen && (
                    ...
                    >
                      ...
                      <span className="auto-run-summary">
                        ...
                        <span>
                          <i>ALLOWANCE</i>
                          <strong>{estimate ? `${safeMinutes} safe min` : 'Reading allowance…'}</strong>
                        </span>
                        <span>
                          <i>CODEX</i>
                          <strong>
                  ...
                  >
                    ...
                    <div className="advanced-run-body">
                      <div className="advanced-run-grid">
                        <section className="advanced-run-section model-reasoning-section">
                          <div className="model-reasoning-toolbar">
                            <span className="advanced-section-label">MODEL AND REASONING</span>
                            <label className={`model-customize-choice${modelSelectionMode === 'manual' ? ' active' : ''}`}>
                              <input
              ...
              <div className="estimate-row">
                ...
                <div className="start-column">
                  ...
                  >
                    <img
                      aria-hidden="true"
                      className="automatic-queue-icon"
                      src={`${import.meta.env.BASE_URL}charizard-button-icon.webp`}
                    />
                    <span>
                      {automaticRunActive
...
async function prepareAutomaticRunPlan({
  ...
  for (let pass = 0; pass < 3; pass += 1) {
    const estimate = await retryTransientLocalRequest(() => client.estimate(quotaRiskMode, recommendation?.model.model));
    const safeMinutes = Math.max(0, Math.floor(estimate.allowedMinutes ?? 0));
