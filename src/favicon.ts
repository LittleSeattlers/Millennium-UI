...
function drawGem(context: CanvasRenderingContext2D, gem: GemArt) {
  ...
  const innerBounds = gem.innerScale
    ? scaledTriangleBounds(x, y, gem.width, gem.height, gem.innerScale)
    : { x: x + inset, y: y + inset, width: gem.width - inset * 2, height: gem.height - inset * 2 };
  const inner = gemPath(gem, innerBounds.x, innerBounds.y, innerBounds.width, innerBounds.height);
...
function scaledTriangleBounds(
  ...
  return {
    x: centroidX - scaledWidth / 2,
    y: centroidY - scaledHeight * 2 / 3,
    width: scaledWidth,
    height: scaledHeight,
  };
}
