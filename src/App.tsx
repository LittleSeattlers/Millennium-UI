...
const selectionStorageKey = 'millennium.problem-selection.v1';
const defaultProblem = problems.find((problem) => problem.id === 'riemann-hypothesis') ?? problems[0];
const defaultRoute = defaultProblem.routes.find((route) => route.id === 'refute') ?? defaultProblem.routes[0];

function readInitialSelection() {
  try {
    const raw = window.sessionStorage.getItem(selectionStorageKey);
    const value = raw ? JSON.parse(raw) as { problemId?: unknown; routeId?: unknown } : null;
    const problem = problems.find((candidate) => candidate.id === value?.problemId) ?? defaultProblem;
    const route = problem.routes.find((candidate) => candidate.id === value?.routeId)
      ?? (problem.id === defaultProblem.id ? defaultRoute : problem.routes[0]);
    return { problem, routeId: route.id };
  } catch {
    return { problem: defaultProblem, routeId: defaultRoute.id };
  }
}
...
export default function App() {
  ...
  const [selected, setSelected] = useState(initialSelection.problem);
  const [routeId, setRouteId] = useState(initialSelection.routeId);
  ...
  useEffect(() => {
    try {
      ...
      // The requested selection still works for this render when storage is unavailable.
    }
  }, [routeId, selected.id]);
