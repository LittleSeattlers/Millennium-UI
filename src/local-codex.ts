...
export type ResearchTaskValueContract = {
  positiveOutcome: string;
  negativeOutcome: string;
  evidenceRequired: string;
  noveltyRequired: string;
...
export type ResearchTask = {
  ...
  attemptCount: number;
  unlockCount?: number;
  valueContract: ResearchTaskValueContract;
};
...
export type AttemptRecord = {
  ...
  completedAt?: string;
  publication?: {
    status: 'claimed' | 'submitted' | 'withheld' | 'failed' | string;
    prUrl?: string | null;
  ...
  researchValue?: {
    status: 'pending' | 'accepted' | 'insufficient' | string;
    outcome?: 'scoped-advance' | 'bounded-negative' | 'reproducibility-result' | 'frontier-refinement' | string;
    reason?: string | null;
  } | null;
};
...
export class LocalCodexClient {
  ...
  static async pairLaunchedConnector(ticket: ConnectorLaunch, timeoutMs = 20_000) {
    ...
    while (Date.now() < deadline) {
      ...
      try {
        ...
        lastError = error;
        if (error instanceof LocalCompanionError && error.status === 401) {
          throw new Error(
            'This approval ticket expired or belongs to an earlier connector session. Click Connect again to create a fresh approval; reinstalling is not required.',
          );
        }
        if (error instanceof LocalCompanionError
          && ![404, 500, 502, 503, 504].includes(error.status)) throw error;
    ...
    throw new Error(
      'The local connector did not complete pairing. Click Connect again to create a fresh approval. Reinstall only if the local approval page no longer opens.',
    );
  }
  ...
  async estimate(riskMode: RiskMode, model?: string | null) {
    const payload = await this.request<{ estimate: QuotaEstimate }>('/quota/estimate', {
      method: 'POST',
      body: JSON.stringify({ riskMode, model: model || undefined }),
    });
